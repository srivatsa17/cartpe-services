...
class OrderSerializer(serializers.ModelSerializer):
    ...
    amount_paid = serializers.DecimalField(
        max_digits=7, decimal_places=2, coerce_to_string=False, read_only=True
    )
    amount_due = serializers.DecimalField(
        max_digits=7, decimal_places=2, coerce_to_string=False, read_only=True
    )
    amount_refundable = serializers.DecimalField(
        default=0, max_digits=7, decimal_places=2, coerce_to_string=False
    )
    user = serializers.SlugRelatedField(slug_field="email", read_only=True)
    user_address = serializers.SlugRelatedField(slug_field="id", queryset=UserAddress.objects.all())
    is_paid = serializers.BooleanField(default=False)
    status = serializers.ChoiceField(
        choices=OrderStatus.ORDER_STATUS_CHOICES, default=OrderStatus.PENDING
    )
    refund_status = serializers.ChoiceField(
        choices=OrderRefundStatus.ORDER_REFUND_STATUS_CHOICES, default=OrderRefundStatus.NA
    )
    method = serializers.ChoiceField(
        choices=OrderMethod.ORDER_METHOD_CHOICES, default=OrderMethod.UPI
    )
    razorpay_order_id = serializers.CharField(
        min_length=1, max_length=50, allow_null=True, allow_blank=True
    )
    razorpay_payment_id = serializers.CharField(
        min_length=1, max_length=50, allow_null=True, allow_blank=True
    )
    razorpay_signature = serializers.CharField(
        min_length=1, max_length=255, allow_null=True, allow_blank=True
    )
    created_at = serializers.DateTimeField(read_only=True, format="%d %b %Y, %H:%M")
    updated_at = serializers.DateTimeField(read_only=True, format="%d %b %Y, %H:%M")
