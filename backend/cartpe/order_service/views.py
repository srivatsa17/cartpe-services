...
class OrderAPIView(generics.GenericAPIView):
    ...
    def get_queryset(self):
        orders = (
            Order.objects.select_related("user", "user_address")
    ...
    def post(self, request):
        ...
        order_items_serializer = OrderItemSerializer(
            data=request.data.get("order_items"), many=True
        )
        payment_serializer = PaymentSerializer(data=request.data.get("payment_details"))

        if (
            serializer.is_valid()
            and order_items_serializer.is_valid()
            ...
            if serializer.validated_data["method"] == OrderMethod.UPI:
                ...
                serializer.validated_data["amount_paid"] = (
                    razorpay_order_details["amount_paid"] / 100
                )
            ...
            order_items = [
                ...
                for order_item in order_items_serializer.validated_data
            ]
...
class OrderByIdAPIView(generics.GenericAPIView):
    ...
    def patch(self, request, id):
        ...
        if serializer.is_valid():
            if (
                serializer.validated_data["status"] == OrderStatus.CANCELLED
                or serializer.validated_data["status"] == OrderStatus.RETURNED
            ):
