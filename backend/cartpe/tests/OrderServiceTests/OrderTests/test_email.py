...
class SendOrderConfirmationEmailTest(TestCase):
    ...
    def test_send_order_confirmation_email_success(self, mock_send):
        ...
        with patch("order_service.email.User.objects.filter") as mock_user_filter, \
            patch("order_service.email.render_to_string") as mock_render_to_string:

            # Mock the render_to_string method to return the HTML content
            mock_render_to_string.return_value = "<html><body>Order Confirmation Email</body></html>"
