...
class UpdateCategoryByIdTest(APITestCase):
    ...
    def setUp(self):
        ...
        self.women = Category.objects.create(
            name="Women", description="Clothing for women", parent=None
        )
        ...
        self.footwear = Category.objects.create(
            name="Footwear", description="Footwear for men", parent=self.men
        )

    def test_update_with_valid_input(self):
        url = self.get_url(self.men.id)
        data = json.dumps({"id": 20, "name": "abc", "description": "good one", "slug": "abcd"})
